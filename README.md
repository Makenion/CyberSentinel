...
Para garantizar la persistencia de los datos en **Limache**, el sistema cuenta con:
- **Respaldos Automáticos**: Ejecución cada 24 horas almacenada en `/backups`.
- **Respaldos Manuales**: Comando `!respaldar` disponible para administradores.
- **Recuperación**: Para restaurar, detenga el contenedor, reemplace `data/sentinel_vault.db` con el archivo de la carpeta `backups/` y reinicie el servicio.
...
| 17             | 17 Ene | Seguridad & Respaldos                         | Implementación de snapshots automáticos de la base de datos cada 24 horas y comando manual !respaldar para asegurar la persistencia                                                                                                  |
---
